...
#define BYTES_PER_SAMPLE   (sizeof(uint32_t))                  /* number of bytes per sample */
#define SAMPLES_PER_SECTOR (BYTES_PER_SECTOR/BYTES_PER_SAMPLE) /* number of samples per sector */
	...
	const bool is_last_track)
{
	...
	if (is_first_track)
	{
		 ...
		 */
		audio_start += (5*SAMPLES_PER_SECTOR-1)*num_channels;
		crc_pos += 5*SAMPLES_PER_SECTOR-1;
		printf("starting at pos %zu, crc_pos=%u\n", (void*)audio_start-(void*)audio_data, crc_pos);
	}
	if (is_last_track)
	{
		/* a the the end, we do skip exactly 5 full sectors */
		audio_end -= 5*SAMPLES_PER_SECTOR*num_channels;
	}
